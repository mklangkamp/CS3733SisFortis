...
java.sql.Connection conn;
    ...
    public TaskDAO(LambdaLogger logger, boolean withoutTeammateToTaskDAO) {
    	this.logger = logger;
//    	this.teammateToTaskDAO = new TeammateToTaskDAO(logger);
//    	this.teammateDAO = new TeammateDAO(logger,false);
    	try  {
    		logger.log("Trying to connect to database");
    		conn = DatabaseUtil.connect();
    		logger.log("Connected to database");
    ...
    public ArrayList<Task> getTasksForProject(Project p) throws Exception {
        try {
      	  	logger.log("getting Tasks for project: " + p.name);
