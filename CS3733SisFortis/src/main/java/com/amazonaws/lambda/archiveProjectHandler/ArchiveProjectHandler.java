package com.amazonaws.lambda.archiveProjectHandler;
...
public class ArchiveProjectHandler implements RequestHandler<ArchiveProjectRequest, ArchiveProjectResponse> {
    ...
    public ArchiveProjectResponse handleRequest(ArchiveProjectRequest req, Context context) {
    	...
    	logger.log("Loading Java Lambda handler of ArchiveProjectHandler");
		logger.log(req.toString());
		
		try {
			dao.archiveProject(req.projectName);
		}
		catch(Exception e){
			System.out.println("Could not archive project.");
		}
		
		ArchiveProjectResponse response = new ArchiveProjectResponse(req.projectName, 200);
